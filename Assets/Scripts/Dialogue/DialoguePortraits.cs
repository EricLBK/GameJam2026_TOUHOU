...
public class DialoguePortraits : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            if (!string.IsNullOrWhiteSpace(ns.key) && ns.sprite != null)
                map[ns.key] = ns.sprite;
        }
    }

    // ✅ NOTE: DialogueLine is nested inside DialogueUI
    public void Apply(DialogueUI.DialogueLine line)
    {
        if (line == null) return;

        // Optional swaps (leave key empty to keep current portrait)
        ...
        // Highlight speaker
        bool leftSpeaking = line.speaker == DialogueUI.Slot.Left;
    ...
    {
        if (!img) return;
        if (map != null && map.TryGetValue(key, out var sp) && sp != null)
            img.sprite = sp;
    }
